...
router.get('/:token', async (req, res) => {
  try {
    const db = getDb()
    const tokenRow = await db.prepare('SELECT * FROM designer_tokens WHERE token=?').get(req.params.token)
    ...
    const result = await Promise.all(parts.map(async (part) => {
      ...
      const hasDefectAlert = totals?.total > 0 && (totals.defects / totals.total) > 0.05
    ...
    res.json({
      product: {
        ...
        order_qty: product.order_qty, order_date: product.order_date,
